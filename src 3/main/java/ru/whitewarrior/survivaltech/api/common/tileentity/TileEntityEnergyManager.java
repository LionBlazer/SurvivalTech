 ...
 * @author WhiteWarrior
 */
public abstract class TileEntityEnergyManager extends TileEntityInventory implements IEnergyElectricNetworkManager, ITickable, IElectricEnergyStorage {
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		.getStorage().setEnergy(compound.getDouble(NBTConstants.energy));
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) {
		...
		compound.setDouble(NBTConstants.energy, .getEnergyStored());
		int i = 0;
	...
	public void update() {
		if (!world.isRemote) {
			if (receiversPos.size() > 0) {
				...
				ElectricEnergyStorage storage = .getStorage();
				...
				if (storage.getEnergyStored() >= storage.getMaxExtract()) {
					...
					Double energyold2 = new Double(storage.getEnergyStored());
					...
					for (Entry<BlockPos, Integer> input : receiversPos.entrySet()) {
						...
						if (.getWorld().getTileEntity(input.getKey()) != null && .getWorld().getTileEntity(input.getKey()) !=
								&& .getWorld().getTileEntity(input.getKey()) instanceof IElectricEnergyStorage) {
							// Тайл потребителя
							IElectricEnergyStorage tile2 = (IElectricEnergyStorage) .getWorld().getTileEntity(input.getKey());
							...
							if (storage2.getEnergyReceivedPerTick() < storage2.getMaxReceive() && storage2.getEnergyStored() < storage2.getMaxEnergyStored()) {
								...
								*/
								...
								if (storage2.getEnergyStored() < storage2.getMaxEnergyStored())
									if (!activeReceiversPos.containsKey(input))
										activeReceiversPos.put(input.getKey(), receiversPos.get(input));

								if (!energyold2.equals(.getStorage().getEnergyStored())) {
									updateVars();
