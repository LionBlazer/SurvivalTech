...
public abstract class TileEntityEnergyManager extends TileEntityEnergyStandard implements IEnergyElectricNetworkManager, ITickable, IElectricEnergyStorage {
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        for (int i = 0; i < compound.getInteger("pos_size"); i++) {
            receiversPos.put(new BlockPos(compound.getInteger("positions_x_" + i), compound.getInteger("positions_y_" + i), compound.getInteger("positions_z_" + i)), compound.getInteger("cond_" + i));

        }

        for (int i = 0; i < compound.getInteger("active_pos_size"); i++) {
            activeReceiversPos.put(new BlockPos(compound.getInteger("activepositions_x_" + i), compound.getInteger("activepositions_y_" + i), compound.getInteger("activepositions_z_" + i)), compound.getInteger("activecond_" + i));
            ...
            if (receiversPos.size() > 0) {
                ...
                if (storage.getEnergyStoredMod() >= storage.getMaxExtract()) {
                    ...
                    for (Entry<BlockPos, Integer> input : receiversPos.entrySet()) {
                        ...
                        if (input != null && input.getKey() != null && this.getWorld().getTileEntity(input.getKey()) != null && this.getWorld().getTileEntity(input.getKey()) != this && this.getWorld().getTileEntity(input.getKey()) instanceof IElectricEnergyStorage) {
                            // Тайл потребителя
                            ...
                            if (storage2.getEnergyReceivedPerTick() < storage2.getMaxReceive() && storage2.getEnergyStoredMod() < storage2.getMaxEnergyStoredMod()) {
								...
								*/
                                ...
                                double energyForEach = Math.min(storage.extractEnergy(storage.getMaxExtract(), true), d01) / activeReceiversPos.size();
                                if (energyForEach - (((double) input.getValue()) / 100d) <= 0)
                                    energyForEach = 0;
